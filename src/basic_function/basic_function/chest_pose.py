...
class ChestPose(Node):
    def __init__(self):
        super().__init__('chest_pose')
        ...
        # 发布 arrow
        self.chest_pose_pub = self.create_publisher(TimeFloat, 'chest_pose', 10)
        self.pose_arrow = self.create_publisher(Marker, 'chest_arrow', 10)
        self.select_points = self.create_publisher(Marker, 'chest_select_points', 10)
    ...
    def subscription_callback(self, msg):
        ...
        if self.depth_image is not None or self.img is not None:
            ...
            # 发布 chest pose
            chest_pose_msg = TimeFloat()
            chest_pose_msg.header = Header()
            chest_pose_msg.header.stamp = self.get_clock().now().to_msg()
            chest_pose_msg.matrix = Float32MultiArray()
            chest_pose_msg.matrix.data = np.hstack((point_pose, normal_pose)).flatten().tolist() # center point + normal vector
